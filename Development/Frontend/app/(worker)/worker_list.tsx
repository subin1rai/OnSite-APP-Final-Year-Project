...
import { ActivityIndicator, FlatList, SafeAreaView, Text, TextInput, TouchableOpacity, View, Pressable, StatusBar, Platform, Image } from "react-native";
...
const WorkerList = () => {
  ...
  useEffect(() => {
    if (!searchText.trim()) {
      ...
      setFilteredWorkers(workers.filter(worker => worker.name.toLowerCase().includes(searchText.toLowerCase())));
    }
  }, [searchText, workers]);
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      >
        <BottomSheetView>
          {isOpen && <MainWorker onWorkerAdded={handleWorkerAdded} />}
        </BottomSheetView>
      </BottomSheet>
    </View>
  );
